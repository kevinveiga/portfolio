...
const button = css<IButton>`
    ...
    ${({ small, height }): string | undefined =>
        height === undefined ? (small ? `height: ${variable.layout.buttonHeight}` : `height: ${variable.layout.buttonHeight}`) : undefined};
...
export const ButtonStyled = styled.button<IButton>`
    ...
    background-color: ${({ active, backgroundColor }): string => (active ? variable.color.primaryHover : backgroundColor || variable.color.primary)};
    ${({ borderRadius }): string | undefined => (borderRadius === undefined ? `border-radius: ${variable.layout.borderRadiusPrimary}` : undefined)};
    color: ${({ active, color }): string => (active ? variable.color.white : color || variable.color.white)};
    transition: background-color ${variable.animation.transition}, border ${variable.animation.transition}, color ${variable.animation.transition};
...
export const ButtonBorderStyled = styled.button<IButton>`
    ...
    background-color: ${({ active, backgroundColor }): string => (active ? variable.color.primaryHover : backgroundColor || 'transparent')};
    border-color: ${({ active, borderColor }): string => (active ? variable.color.primaryHover : borderColor || variable.color.primary)};
    ${({ borderRadius }): string | undefined => (borderRadius === undefined ? `border-radius: ${variable.layout.borderRadiusPrimary}` : undefined)};
    border-style: solid;
    border-width: 2px;
...
export const ButtonUnsetStyled = styled.button<IButton>`
    ...
    span,
    svg {
        ${({ verticalAlign }): string | undefined => (verticalAlign === undefined ? 'vertical-align: baseline' : undefined)};
    }

    p,
    span {
        ${({ active, color }): string | undefined => (active || color ? `color: ${active ? variable.color.primary : color || 'unset'}` : undefined)};
        transition: color ${variable.animation.transition};
    }

    svg {
        ${({ active, color }): string | undefined => (active || color ? `fill: ${active ? variable.color.primary : color || 'unset'}` : undefined)};
        transition: fill ${variable.animation.transition};
    }
`;
