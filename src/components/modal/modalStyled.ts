...
export const ModalTopStyled = styled.div<IModalStyled>`
    ...
    background-color: ${({ type }): string =>
        type === 'error' ? variable.color.secondary : type === 'success' ? variable.color.tertiary : type || variable.color.tertiary};
    color: ${({ type, theme }): string => (type === 'error' || type === 'success' ? variable.color.black : type || theme.textColor.primary)};
    display: flex;
    flex: 1 1 auto;
    flex-wrap: wrap;
