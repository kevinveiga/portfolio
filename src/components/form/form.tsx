...
export function LabelFile({ ariaLabel, children, component: Component = LabelFileStyled, forLabel, text, ...props }: ILabel): ReactElement {
    const acessibility = ariaLabel || text;
    const labelContent = children || text;
        ...
        if (inputCleanValueRef.current && inputFormattedValueRef.current) {
            try {
                ...
                if (validationSchema) {
                    ...
                    validationSchema.validateSync(
                        {
                            [fieldName]: maskDecimal({ decimalScale: decimalScale, decimalSeparator: decimalSeparator, value: formattedValue })
                                .formattedValue
                        },
                        {
                            abortEarly: false
    ...
    return (
        <InputDecrementIncrementContainerStyled>
            ...
            <Button
                height="12px"
                onClick={(): void =>
                    handleChange(parseInt((inputFormattedValueRef?.current?.value || 0).toString().replace(/\D/g, ''), 10) + stepFactor)
                }
...
export function Textarea({ cbFunction, idInput, label, maxLength = 1000, name, placeholder, validationSchema, ...props }: IInput): ReactElement {
    // REF
