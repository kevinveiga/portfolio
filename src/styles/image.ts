...
export const BgImage = styled.div<IImageStyled>`
    ...
    ${({ backgroundPosition }): string | undefined => (backgroundPosition === undefined ? 'background-position: 50% 50%' : undefined)};
    ${({ backgroundRepeat }): string | undefined => (backgroundRepeat === undefined ? 'background-repeat: no-repeat' : undefined)};
